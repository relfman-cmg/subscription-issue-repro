...
`{"type":"connection_ack"}` comes back on the socket and `next` frames arrive every 5s, so
the subscription is demonstrably live end to end. Then freeze the subgraph:
...
`next` frames stop at the freeze and never resume. Nitro reports no error and no completion;
the subscription simply sits there, and `graphql-transport-ws` pings keep arriving. Nothing
appears in the gateway log. Left running, it stays that way indefinitely.
